...
# Send retry mechanism for unacknowledged messages
class RetryAsyncCommand:
    # lancaigang231219: changed
    # Timeout 5 seconds
    # TIMEOUT_TIME = 5.0
    TIMEOUT_TIME = 10.0
    # Retry time 500 milliseconds
    # RETRY_TIME = 0.500
    RETRY_TIME = 1.0

    def __init__(self, serial, name, oid=None):
        # Toolhead board serial port
        self.serial = serial
        self.name = name
        self.oid = oid
        ...
        # Callback function; receive data processing
        self.serial.register_response(self.handle_callback, name, oid)

        # #printer.cfg configuration file
        ...
        while 1:
            ...
            if query_time > first_query_time + self.TIMEOUT_TIME:
                ...
                raise serialhdl.error(
                    "[(Phrozen)Klipper]Timeout on wait for '%s' response" % (self.name,)
                )
            # Send to toolhead board serial port
            self.serial.raw_send(cmd, minclock, minclock, cmd_queue)


# Wrapper around query commands
# Wrapper for query commands
class CommandQueryWrapper:
    ...
    def send_with_preface(self, preface_cmd, preface_data=(), data=(), minclock=0, reqclock=0):
...
class MCU_trsync:
    ...
    def _build_config(self):
        ...
        mcu.add_config_cmd(
            "trsync_start oid=%d report_clock=0 report_ticks=0 expire_reason=0" % (self._oid,),
            on_restart=True,
        )
        # Lookup commands
    ...
    def start(self, print_time, trigger_completion, expire_timeout):
        ...
        min_extend_ticks = self._mcu.seconds_to_clock(expire_timeout * 0.4 * 0.8)
        ffi_main, ffi_lib = chelper.get_ffi()
        ...
        self._trsync_start_cmd.send(
            [self._oid, clock, report_ticks, self.REASON_COMMS_TIMEOUT], reqclock=clock
        )
...
class MCU_endstop:
            ...
            for ot in self._trsyncs:
                for s in ot.get_steppers():
                    if ot is not trsync and s.get_name().startswith(sname[:9]):
                        ...
                        raise cerror("Multi-mcu homing not supported on" " multi-mcu shared axis")
    ...
    def _build_config(self):
        ...
        self._mcu.add_config_cmd(
            "config_endstop oid=%d pin=%s pull_up=%d" % (self._oid, self._pin, self._pullup)
        )
        self._mcu.add_config_cmd(
...
class MCU_digital_out:
    ...
    def _build_config(self):
        ...
        if self._max_duration and self._start_value != self._shutdown_value:
            raise pins.error(
                "[(Phrozen)Klipper]Pin with max duration must have start"
...
class MCU_pwm:
    ...
    def _build_config(self):
        ...
        if self._hardware_pwm:
            ...
            self._mcu.add_config_cmd(
                "queue_pwm_out oid=%d clock=%d value=%d" % (self._oid, self._last_clock, svalue),
                on_restart=True,
            )
            self._set_cmd = self._mcu.lookup_command(
        ...
        if self._is_static:
            self._mcu.add_config_cmd(
                "set_digital_out pin=%s value=%d" % (self._pin, self._start_value >= 0.5)
            )
            return
        if cycle_ticks >= 1 << 31:
        ...
        self._mcu.add_config_cmd(
            "queue_digital_out oid=%d clock=%d on_ticks=%d" % (self._oid, self._last_clock, svalue),
            is_init=True,
        )
        self._set_cmd = self._mcu.lookup_command(
            "queue_digital_out oid=%c clock=%u on_ticks=%u", cq=cmd_queue
    ...
    def set_pwm(self, print_time, value, cycle_time=None):
        ...
        if self._hardware_pwm:
            ...
            self._set_cmd.send([self._oid, clock, v], minclock=minclock, reqclock=clock)
            return
        # Soft pwm update
        ...
        if cycle_ticks != self._last_cycle_ticks:
            ...
            self._set_cycle_ticks.send([self._oid, cycle_ticks], minclock=minclock, reqclock=clock)
            self._last_cycle_ticks = cycle_ticks
...
class MCU_adc:
    ...
    def setup_minmax(self, sample_time, sample_count, minval=0.0, maxval=1.0, range_check_count=0):
...
class MCU:
    ...
    def __init__(self, config, clocksync):
        ...
        self._max_stepper_error = config.getfloat("max_stepper_error", 0.000025, minval=0.0)
        ...
        # self.G_PhrozenGCode.respond_info("[(mcu.py)__init__]printer.lookup_object(\"gcode\")")
        ...
        try:
            ...
            if enum_name == "pin":
                ...
                raise self._printer.config_error(
                    "Pin '%s' is not a valid pin name on mcu '%s'" % (enum_value, self._name)
                )
            raise
    ...
    def _send_get_config(self):
        ...
        if self._is_shutdown:
            raise error(
                "[(Phrozen)Klipper]MCU '%s' error during config: %s"
                % (self._name, self._shutdown_msg)
            )
        if config_params["is_shutdown"]:
            raise error(
                "[(Phrozen)Klipper]Can not update MCU '%s' config as it is shutdown" % (self._name,)
            )
    ...
    def _connect(self):
        ...
        if not config_params["is_config"]:
            ...
            start_reason = self._printer.get_start_args().get("start_reason")
            if start_reason == "firmware_restart":
                raise error("[(Phrozen)Klipper]Failed automated reset of MCU '%s'" % (self._name,))
        ...
        if self.is_fileoutput():
            ...
            try:
                if self._canbus_iface is not None:
                    ...
                    self._serial.connect_canbus(self._serialport, nodeid, self._canbus_iface)
                elif self._baud:
    ...
    def flush_moves(self, print_time):
        ...
        if ret:
            raise error("[(Phrozen)Klipper]Internal error in MCU '%s' stepcompress" % (self._name,))
    ...
    def check_active(self, print_time, eventtime):
        ...
        # lancaigang250901：
        logging.info(
            "=====phrozen=====[(mcu.py)check_active]Timeout with MCU '%s' (eventtime=%f)",
            self._name,
            eventtime,
        )
        self.G_PhrozenGCode.respond_info("=====phrozen=====[(mcu.py)check_active]Timeout with MCU")
        # lancaigang250901: disabled
        # self._is_timeout = True
        # lancaigang250901: shutdown printer; disabled
        # self._printer.invoke_shutdown("Lost communication with MCU '%s'" % (self._name,))
        logging.info(
            "=====phrozen=====[(mcu.py)check_active]Lost communication with MCU '%s' (eventtime=%f)",
            self._name,
            eventtime,
        )
        self.G_PhrozenGCode.respond_info(
            "=====phrozen=====[(mcu.py)check_active]Lost communication with MCU"
        )
    ...
    def stats(self, eventtime):
        load = (
            "[(Klipper)mcu.py]klipper-to-toolhead heartbeat; mcu_awake=%.03f mcu_task_avg=%.06f mcu_task_stddev=%.06f"
            % (self._mcu_tick_awake, self._mcu_tick_avg, self._mcu_tick_stddev)
        )
        stats = " ".join([load, self._serial.stats(eventtime), self._clocksync.stats(eventtime)])
        ...
        # lancaigang231219: log printing disabled
        # logging.info(stats)
...
def add_printer_objects(config):
    ...
    # G_PhrozenFluiddRespondInfo("[(mcu.py)add_printer_objects]printer adds mcu")
